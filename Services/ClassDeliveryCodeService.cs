...
namespace SnackUpAPI.Services
{
    public class ClassDeliveryCodeService
    {
        ...
        public ClassDeliveryCodeService(IDatabaseService databaseService)
        {
        ...
        public IEnumerable<PDFCode> GetPDFCode(string recreation, string deliveryDate)
        {
            return _databaseService.Query<PDFCode>(
                @"
...
SELECT DISTINCT
    ...
    SC.ClassSection,
    CASE 
        WHEN @Recreation = 'First' THEN CDC.Code1
        WHEN @Recreation = 'Second' THEN CDC.Code2
    END AS PickupCode
FROM Orders AS O
INNER JOIN SchoolClasses AS SC ON O.SchoolClassID = SC.SchoolClassID
INNER JOIN ClassDeliveryCodes AS CDC ON SC.SchoolClassID = CDC.SchoolClassID
WHERE 
    EXISTS (
        SELECT 1
        ...
        WHERE 
            ...
            AND OD.DeliveryDate = @DeliveryDate
            AND OD.Recreation = @Recreation
    )
ORDER BY SC.ClassYear, SC.ClassSection;",
                new { Recreation = recreation, DeliveryDate = deliveryDate }
            );
        }
        ...
        public CodeStatusResponse GetCodeStatusProducer(int ClassYear, string ClassSection, string currentMoment)
        {
            Debug.WriteLine($"Momento in entrata: {currentMoment}");
            ...
            string query = currentMoment switch
            {
                "First" => @"SELECT TOP 1 
                  ...
                  FROM 
                      ClassDeliveryCodes AS CD
					  inner join SchoolClasses U on CD.SchoolClassID= U.SchoolClassID
                  WHERE 
				  U.ClassYear= @ClassYear AND 
				  U.ClassSection=@ClassSection AND
				  CD.Deleted IS NULL",
                "Second" => @"SELECT TOP 1 
                      CD.RetrievedCode2 AS IsRetrieved 
                  FROM 
            ...
            try
            {
                return _databaseService.QuerySingle<CodeStatusResponse>(query, new { ClassYear = ClassYear, ClassSection= ClassSection });
            }
            catch (Exception ex)
            {
                Debug.WriteLine($"Error retrieving code status: {ex.Message}");
        ...
        public async Task<string> IsCodeAlreadyRetrievedWithName(int userID, string currentMoment)
        {
            string query = currentMoment switch
            {
                 "First" => @"
                    SELECT 
                        CASE 
                            WHEN CD.RetrievedCode1 = 1 THEN CONCAT(U.UserName, ' ', U.Surname)
                            ELSE 'non ritirato'
                        END AS Status
                    FROM 
                        ClassDeliveryCodes AS CD
                ...
                "Second" => @"
                    SELECT 
                        CASE 
                            WHEN CD.RetrievedCode2 = 1 THEN CONCAT(U.UserName, ' ', U.Surname)
                            ELSE 'non ritirato'
                        END AS Status
                    FROM 
                        ClassDeliveryCodes AS CD
