...
public class RefreshTokenService
{
    ...
    public RefreshTokenService(DatabaseService databaseService, int refreshTokenExpiryInDays)
    {
    ...
    {
        Debug.WriteLine($"Validazione del Refresh Token per UserId: {userId}");
        Debug.WriteLine($"Token ricevuto: {token}");

        var refreshToken = _databaseService.QuerySingle<dynamic>(
        ...
        {
            Console.WriteLine("Token non trovato o non valido.");
        }
        else
        {
            Console.WriteLine("Token trovato e valido.");
        }
