...
public class ClassDeliveryCodeController : ControllerBase
{
    ...
    public IActionResult GetClassDeliveryCodesWithOrders()
    {
        try
        {
            ...
            var result = deliveryCodes.Select(dc => new
            {
                ...
                {
                    order.OrderID,
                    UserName = _classDeliveryCodeService.GetUserById(order.UserID).UserName,
                    order.TotalPrice,
                    order.Status
                }).ToList()
            }).ToList();
    ...
    [HttpGet("GetPDFCodes/{recreation}/{deliveryDate}")]
    public IActionResult GetPDFCode(string recreation, string deliveryDate)
    {
        var result = _classDeliveryCodeService.GetPDFCode(recreation, deliveryDate);
        return Ok(result);
    }
    ...
    [HttpGet("CheckCodeStatusProducer/{moment}/{classYear}/{classSection}")]
    public IActionResult CheckCodeStatusProducer(int classYear, string classSection , string moment)
    {
        var status = _classDeliveryCodeService.GetCodeStatusProducer(classYear,classSection, moment);
        if (status.IsRetrieved)
        {
            return Ok(new { IsRetrieved = true });
        }
