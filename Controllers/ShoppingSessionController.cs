...
public class ShoppingSessionController : ControllerBase
{
    ...
    public IActionResult CompleteSession(int userId)
    {
        try
        {
            var session = _sessionService.GetActiveSessionForUser(userId);
            if (session == null)
            {
                return NotFound($"Nessuna sessione attiva trovata per l'utente con ID {userId}.");
            }

            _sessionService.CompleteSession(userId);
            return Ok(new { Message = "Sessione completata con successo." });
        }
        catch (Exception ex)
        {
            Console.WriteLine($"Errore durante il completamento della sessione: {ex.Message}");
            return StatusCode(500, "Errore interno del server.");
        }
    }
